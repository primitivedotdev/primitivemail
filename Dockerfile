...
RUN apt-get update && \
    apt-get install -y --no-install-recommends \
      ...
      libmilter-dev \
      iproute2 && \
    pip3 install --no-cache-dir python-logging-loki pymilter prometheus_client 'pyspf>=2.0,<3' 'dnspython>=2.7,<3' 'dkimpy>=1.1,<2' 'tldextract>=5.0,<6' 'boto3>=1.35,<2' 'urllib3>=2.0,<3' && \

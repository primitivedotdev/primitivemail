...
def run_with_progress(
    ...
    # Skip the braille spinner when stderr isn't a TTY. The spinner uses
    # `\r` to redraw in place — works on a terminal, but when stderr is
    # redirected to a log file or piped through SSH without a pty, each
    # frame becomes a new line and the log fills with thousands of
    # "Building (Ns)" frames (a naive agent running over SSH saw ~1100).
    # Emit a single start line, capture output silently, and report
    # complete / fail just like the JSON-mode path does.
